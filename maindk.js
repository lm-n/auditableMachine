let generated = [];
const noImage = "Kan ikke generere det billede lige nu! Prøv en anden kombination.";
let imageData = [];
    ...
    if (generated.length === 0) {
        gallery.innerHTML = `
            <div style="grid-column: 1/-1; text-align: center; color: #5f6368; padding: 60px 20px;">
                ...
                <p>Klik på <strong>Opret billede</strong> for at generere dit første billede.</p>
            </div>`;
        return;
    }
    ...
    if (data.length === 0) {
        gallery.innerHTML = `
            <p style="grid-column: 1/-1; text-align: center; color: #777; padding: 40px;">
                Ingen billeder i dit datasæt matcher disse specifikke filtre.
            </p>`;
        return;
    }
...
generateBtn.addEventListener('click', () => {
    ...
    if (!selectedAnimal || !selectedSetting || !selectedWeather) {
        alert("Vælg venligst alle valgmuligheder!");
        return;
    }
